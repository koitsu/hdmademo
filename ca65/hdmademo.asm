...
; If using ca65 v2.17 or older, the below BANKxx segments must be
; declared **BEFORE** any underlying code references them.  Otherwise, their
; assemble-time addresses will be incorrect (the assembler will try to
; determine them, rather than letting ld65 fill them in later ("rr" in a
; listing file)).  To ensure they're correct, refer to the .lbl file.
;
; An alternate approach is to put the BANKxx segments last and then declare
; a forward reference to every single label in those banks at the top of
; the file.
;
; This bug is rectified in ca65 v2.18 or newer.

; Below graphics (.DAT files) were generated by older MS-DOS-based tools,
; most likely GIF2SNES.EXE, GIF2SOPT.EXE ("SNES optimised"), or RAW2SNES
; (the latter was a utility also made by Norman Yen).
;
...
;
; These should really be regenerated using newer tools like Superfamiconv:
